...
resource "aws_synthetics_canary" "api_health" {
  name                 = "fastapi-${var.environment}-api-health"
  artifact_s3_location = "${local.s3_base_path}/api-health/"
  ...
  start_canary         = true
  schedule {
    expression = local.canary_schedule
  }
  run_config {
    timeout_in_seconds = 30
    environment_variables = {
      EXPECTED_SUBSTRING = "${var.environment}"
      TARGET_URL         = "${local.api_url_base}/health"
...
resource "aws_synthetics_canary" "user_profile" {
  name                 = "fastapi-${var.environment}-api-user-profile"
  ...
  run_config {
    ...
    environment_variables = {
      LOGIN_URL = "${local.api_url_base}/api/v1/auth/login"
      ME_URL    = "${local.api_url_base}/api/v1/user/me"
