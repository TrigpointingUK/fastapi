...
def test_admin_get_any_email_success(
    client: TestClient, test_user, test_admin_user, admin_token
):
...
def test_user_get_other_email_forbidden(
    client: TestClient, test_user, test_admin_user, user_token
):
...
def test_get_email_nonexistent_user(client: TestClient, user_token):
    ...
    headers = {"Authorization": f"Bearer {user_token}"}
    response = client.get(f"{settings.API_V1_STR}/users/email/999999", headers=headers)
